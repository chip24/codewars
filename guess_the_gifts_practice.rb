#I solved this on my own without help from AI.  The top answer on CodeWars below was much shorter.
...
def guess_gifts(wishlist, presents)
  ...
  match_arr.map { |str| str.gsub(/\d/, "") }.uniq
end

p guess_gifts([{:name=>"socks", :size=>"small", :clatters=>"no", :weight=>"light"}, {:name=>"card game", :size=>"small", :clatters=>"no", :weight=>"light"}, {:name=>"bobble hat", :size=>"small", :clatters=>"no", :weight=>"light"}], [{:size=>"small", :clatters=>"no", :weight=>"light"}, {:size=>"small", :clatters=>"no", :weight=>"light"}])
