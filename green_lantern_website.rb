# My code was ranked 3 out of 54.  I needed to Google how to switch from decimal to binary and how to break strings into chunks using scan.
...
def yellow_be_gone(color_name_or_code)
  ...
  if downcase_hash.has_key?(downcase_code)
    ...
    color_code = color_name_or_code[1..-1]
    colors_hex = color_code.scan(/.{2}/)
    colors_dec =  color_code.scan(/.{2}/).map {|color| color.to_i(16)}
    hex_sorted = colors_hex.sort_by {|hex_string| hex_string.to_i(16) }
